...
const populateEvent = async (query) => {
  ...
  //   model: User,
  //   select:
  //     "_id title discription location createdAt imageUrl startDateTime endDateTime price isFree url",
  // });
};

const populateTicket = async (query) => {
...
export const getEventsTickets = async function (
  userId,
  eventId,
  limit = 3,
  page = 1
  ...
  try {
    ...
    const events = await populateTicket(eventsQuery);
    const eventsCount = await Event.countDocuments(conditions);
